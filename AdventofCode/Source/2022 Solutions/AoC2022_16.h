...
template<> Number AoC<2022, 17, B>(std::istream& input)
{
  ...
  for (const auto& line : v)
  {
    ...
    char w3[9];
    char n1[4], n2[4], n3[4], n4[4], n5[4], n6[4];
    int z = sscanf_s(line.c_str(), "Valve %s has flow rate=%d; %s %s to %s %[^,], %[^,], %[^,], %[^,], %[^,], %[^,]", n, 3, &valve.flow, w1, 8, w2, 8, w3, 8, n1, 3, n2, 3, n3, 3, n4, 3, n5, 3, n6, 3);
    switch (z)
    {
      default:
        break;
  ...
  State root{ 0,0,-1,-1,std::vector<bool>(valves.size(), false) };
  root.currentM = root.currentE = valvemap["AA"];
  ...
  struct shash
  {
    size_t operator() (const State& s) const noexcept
    {
      return std::hash<int>()(s.currentM) + std::hash<int>()(s.currentE*57) + std::hash<std::vector<bool>>()(s.opened) * 113 * 37;
    }
  };
  struct sequal
  {
    bool operator() (const State& s1, const State& s2) const noexcept
    {
      return s1.currentM == s2.currentM && s1.currentE == s2.currentE && s1.opened == s2.opened;
    }
  };
  ...
  while (z-- > 0)
  {
    ...
    for (auto& s : states)
    {
      if (!s.opened[s.currentM] && valves[s.currentM].flow > 0)  // option: open this one
      {
        State ss = s;
        ss.opened[ss.currentM] = true;
        ss.Released += ss.Open;
        ss.Open += valves[ss.currentM].flow;

        if (!s.opened[s.currentE] && valves[s.currentE].flow > 0)  // option: Elephant opens his
        {
          State sss = ss;
          sss.opened[s.currentE] = true;
          sss.Open += valves[s.currentE].flow;
          H::Update(newstates, sss, z);
        }
        for (int t : valves[s.currentE].tunnels)  // options Elephant walk in each tunnel
        {
          if (t == ss.currentM) continue; // don't go to the same place
          if (ss.opened[t]) continue; // someone already went there
          State sss = ss;
          sss.currentE = t;
          if (valves[t].flow == 0) sss.opened[t] = true; // treat visited like opened for zero flows
          H::Update(newstates, sss, z);
        }
      }
      for (int t : valves[s.currentM].tunnels)  // options walk in each tunnel
      {
        if (t == s.currentE) continue; // don't go to the same place
        if (s.opened[t]) continue; // someone already went there
        State ss = s;
        ss.currentM = t;
        if (valves[t].flow == 0) ss.opened[t] = true; // treat visited like opened for zero flows
        ss.Released += ss.Open;
